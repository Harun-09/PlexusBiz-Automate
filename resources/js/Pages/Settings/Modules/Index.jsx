...
function SummaryCard({ label, value, tone }) {
    return (
        <section className={`rounded-2xl border p-5 shadow-sm ${summaryTone[tone] ?? summaryTone.total}`}>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="max-w-3xl">
                    ...
                    <h3 className="mt-3 text-lg font-black tracking-[-0.03em] text-gray-950">{module.name}</h3>
                ...
                <div className="flex items-center gap-4 lg:min-w-[220px] lg:justify-end">
                    <div className="text-right">
                        <p className="text-xs font-black uppercase tracking-[0.22em] text-gray-400">State</p>
                        <p className="mt-1 text-sm font-semibold text-gray-900">{checked ? 'Enabled' : 'Disabled'}</p>
                    </div>
...
export default function ModuleSettingsIndex({ auth, modules, summary, flash }) {
    ...
    return (
        <AuthenticatedLayout
            ...
            header={
                <div>
                    <p className="text-[11px] font-black uppercase tracking-[0.24em] text-slate-500">Platform control</p>
                    <h2 className="mt-1 text-2xl font-black tracking-[-0.04em] text-gray-950">Module Settings</h2>
                    <p className="mt-1 text-sm text-gray-600">
                        Toggle modules from the database. Locked modules stay enabled because this control panel depends on them.
                    </p>
                </div>
            }
        >

...
export default function Edit({ auth, flash, errors, post: socialPost, platforms = [], statuses = [], accounts = [], campaigns = [] }) {
    const { data, setData, put, processing } = useForm({
        ...
        shares_count: socialPost.shares_count || 0,
    });

    const submit = (event) => {
        event.preventDefault();
        put(route('social.posts.update', socialPost.id), { preserveScroll: true });
    };
        ...
        <AuthenticatedLayout
            ...
            header={
                <PageHeader
                    ...
                    description="Update platform, schedule, status, and engagement placeholders."
                    actions={
                        <Link href={route('social.posts.index')} className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 transition hover:border-sky-200 hover:text-sky-800">
                            Back to posts
                        </Link>
                    }
                />
                ...
                <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
                    <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
                        <div className="border-b border-slate-100 bg-slate-50 px-5 py-5 sm:px-8">
                            ...
                            <h2 className="mt-1 text-2xl font-black tracking-tight text-slate-950">Update scheduled content</h2>
                        </div>
                        ...
                        <div className="px-5 py-6 sm:px-8">
                            ...
                            <form onSubmit={submit} className="mt-6 space-y-5">
                                ...
                                <Field label="Content" error={errors.content}>
                                    <textarea value={data.content} onChange={(event) => setData('content', event.target.value)} className="input min-h-[160px]" required />
                                </Field>

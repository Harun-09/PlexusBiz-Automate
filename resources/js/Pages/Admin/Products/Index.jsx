...
export default function ProductsIndex({ products, filters, statuses, suppliers, flash }) {
    ...
    const submitFilters = (e) => {
        e.preventDefault();
        router.get('/admin/products', Object.fromEntries(Object.entries(data).filter(([, value]) => value !== '')), {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    return (
        ...
        >
            ...
            <div className="space-y-6">
                ...
                {flash?.error && (
                    <div className="rounded-xl border border-rose-200 bg-rose-50 px-5 py-3 text-sm font-medium text-rose-800">
                        {flash.error}
                    </div>
                )}
                ...
                <form onSubmit={submitFilters} className="rounded-2xl border border-gray-200/80 bg-white p-5 shadow-sm">
                    <div className="grid gap-4 md:grid-cols-[1fr_180px_200px_auto] md:items-end">
                        <div>
                            <label htmlFor="product-search" className="block text-xs font-bold uppercase tracking-wider text-gray-500">
                                Search
                            </label>
                            <input
                                id="product-search"
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row">
                            <button
                                type="submit"
                                className="inline-flex h-10 w-full items-center justify-center rounded-xl bg-gray-950 px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-gray-800 sm:w-auto"
                            >
                                Apply
                            </button>
                            <button
                                ...
                                className="inline-flex h-10 w-full items-center justify-center rounded-xl border border-gray-200 bg-white px-5 text-sm font-semibold text-gray-600 shadow-sm transition hover:bg-gray-50 sm:w-auto"
                            >
                                Reset
                            </button>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-100 text-sm">
                                <thead className="bg-gray-50/80">
                                    <tr>
                                        {['SKU', 'Name', 'Supplier', 'Price', 'Stock', 'MOQ', 'Status', 'Actions'].map((heading) => (
                                            <th
                                                key={heading}
                                                className="whitespace-nowrap px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wider text-gray-500"
                                            >
                                                {heading}
                                            </th>
                                        ))}
                                ...
                                <tbody className="divide-y divide-gray-50 bg-white">
                                    {products.data.map((product) => (
                                        <tr key={product.id} className="transition hover:bg-blue-50/30">
                                            <td className="whitespace-nowrap px-6 py-4 font-mono text-xs font-semibold text-gray-600">
                                                {product.sku}
                                            </td>
                ...
                >
                    ...
                    >
                        ...
                        <p className="mt-2 text-center text-sm text-gray-500">
                            This product will be soft-deleted and removed from the active catalog. Continue?
                        </p>
                        <div className="mt-6 flex gap-3">
                            ...
                            <button
                                ...
                                className="flex-1 rounded-xl bg-rose-600 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-rose-700"
                            >
                                Delete
                            </button>
                        </div>

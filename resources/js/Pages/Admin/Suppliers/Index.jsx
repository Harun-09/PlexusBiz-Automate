...
const statusTone = (value) => {
    ...
    if (['approved'].includes(n)) return 'border-emerald-400/30 bg-emerald-500/10 text-emerald-400';
    if (['suspended', 'rejected'].includes(n)) return 'border-rose-400/30 bg-rose-500/10 text-rose-400';
    if (['pending'].includes(n)) return 'border-amber-400/30 bg-amber-500/10 text-amber-400';
...
export default function SuppliersIndex({ auth, suppliers, filters, statuses, flash }) {
                ...
                <div className="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
                    ...
                    <form onSubmit={submitFilters} className="rounded-2xl border border-gray-200/80 bg-white p-5 shadow-sm">
                        <div className="grid gap-4 md:grid-cols-[1fr_200px_auto] md:items-end">
                            <div>
                                <label htmlFor="supplier-search" className="block text-xs font-bold uppercase tracking-wider text-gray-500">Search</label>
                                <input id="supplier-search" type="search" value={data.search} onChange={(e) => setData('search', e.target.value)}
                            ...
                            <div>
                                <label htmlFor="supplier-status" className="block text-xs font-bold uppercase tracking-wider text-gray-500">Status</label>
                                <select id="supplier-status" value={data.status} onChange={(e) => setData('status', e.target.value)}
                    ...
                    <section className="overflow-hidden rounded-2xl border border-gray-200/80 bg-white shadow-sm">
                        <div className="flex flex-col gap-3 border-b border-gray-100 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <h3 className="text-base font-bold text-gray-950">Suppliers</h3>
                                <p className="mt-0.5 text-sm text-gray-500">{suppliers.total} total suppliers</p>
                            </div>
                        </div>

                        {suppliers.data.length === 0 ? (
                            <div className="px-6 py-16 text-center text-sm text-gray-400">No suppliers found.</div>
                        ) : (
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-100 text-sm">
                                    <thead className="bg-gray-50/80">
                                        <tr>
                                            {['Company', 'Owner', 'Email', 'Phone', 'Status', 'Approved', 'Actions'].map((h) => (
                                    ...
                                    <tbody className="divide-y divide-gray-50 bg-white">
                                        {suppliers.data.map((supplier) => (
                                            <tr key={supplier.id} className="transition hover:bg-blue-50/30">
                                                <td className="whitespace-nowrap px-6 py-4 font-medium text-gray-900">{supplier.company_name}</td>
                                                <td className="whitespace-nowrap px-6 py-4 text-gray-600">{supplier.owner || '-'}</td>
                                                <td className="whitespace-nowrap px-6 py-4 text-gray-600">{supplier.contact_email}</td>
                                                <td className="whitespace-nowrap px-6 py-4 text-gray-500">{supplier.phone || '-'}</td>
                                                <td className="whitespace-nowrap px-6 py-4"><StatusPill status={supplier.status} /></td>
                                                ...
                                                <td className="whitespace-nowrap px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <button onClick={() => setShowDeleteModal(supplier.id)}
                                                            className="inline-flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-semibold text-gray-700 shadow-sm transition hover:border-rose-300 hover:text-rose-600">
                        ...
                        {suppliers.last_page > 1 && (
                            <div className="flex flex-col gap-3 border-t border-gray-100 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
                                <p className="text-sm text-gray-500">Page {suppliers.current_page} of {suppliers.last_page}</p>
                                <div className="flex flex-wrap gap-2">
                                    {suppliers.links.filter((l) => l.url).map((link, i) => (
            ...
            {showDeleteModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/50 backdrop-blur-sm" onClick={() => setShowDeleteModal(null)}>
                    <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
                        ...
                        <h3 className="mt-4 text-center text-lg font-bold text-gray-950">Delete Supplier</h3>
                        <p className="mt-2 text-center text-sm text-gray-500">This will soft-delete the supplier and their catalog access. Continue?</p>
                        <div className="mt-6 flex gap-3">
                            ...
                            <button onClick={deleteSupplier} className="flex-1 rounded-xl bg-rose-600 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-rose-700">Delete</button>
                        </div>
                    </div>
                </div>
            )}
